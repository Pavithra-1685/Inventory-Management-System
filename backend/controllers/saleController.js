...
exports.getSales = async (req, res) => {
  ...
  const [sales, total] = await Promise.all([features.query, Sale.countDocuments()]);
  res.status(200).json({ success: true, count: sales.length, total, data: sales });
};
...
exports.createSale = async (req, res) => {
  ...
  await ActivityLog.create({
    user: req.user._id, action: 'create', module: 'sale',
    description: `Created sale invoice: ${sale.invoiceNumber}`,
    entityId: sale._id, entityType: 'Sale', ipAddress: req.ip
  });
...
exports.generateInvoice = async (req, res) => {
  ...
  sale.items.forEach((item, i) => {
    ...
    doc.text(`₹${item.unitPrice.toFixed(2)}`, 355, y);
    doc.text(`₹${(item.discount || 0).toFixed(2)}`, 415, y);
  ...
  doc.text('Subtotal:', 350, y); doc.text(`₹${sale.subtotal.toFixed(2)}`, 480, y);
  y += 15;
  doc.text(`Tax (${sale.taxRate}%):`, 350, y); doc.text(`₹${sale.tax.toFixed(2)}`, 480, y);
  y += 15;
  doc.text('Discount:', 350, y); doc.text(`₹${sale.discount.toFixed(2)}`, 480, y);
  y += 5;
  doc.rect(350, y, 195, 2).fillAndStroke('#111827', '#111827');
  y += 8;
  doc.font('Helvetica-Bold').fontSize(12).fillColor('#111827');
  doc.text('TOTAL:', 350, y); doc.text(`₹${sale.totalAmount.toFixed(2)}`, 480, y);
  y += 18;
  doc.fontSize(9).font('Helvetica').fillColor('#374151');
  doc.text(`Amount Paid: ₹${sale.amountPaid.toFixed(2)}`, 350, y);
  y += 14;
  doc.font('Helvetica-Bold').fillColor(sale.dueAmount > 0 ? '#991B1B' : '#166534');
  doc.text(`Balance Due: ₹${sale.dueAmount.toFixed(2)}`, 350, y);
